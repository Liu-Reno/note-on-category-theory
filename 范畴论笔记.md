...
既有$h_C$为全忠实函子，于是对$\mathcal{C}^{\land}$做出以下限制$h_C(\text{Ob}(\mathcal{C})) = \text{Ob}(\mathcal{C}'^{\land})$,取全子范畴即可得知$\mathcal{C}'^{\land}\xrightarrow{\sim} \mathcal{C}$，即$h_C$为该全子范畴到$\mathcal{C}$的范畴等价，由$h_C$所确定的全子范畴可以将$\mathcal{C}$中的对象$S$转化到$\mathcal{C}^{\land}$中，并且$S$可以由$h_C(S)$唯一确定，即$h_C(S)$可以表出范畴$\mathcal{C}$中的对象$S$.
...
由于两个函子同构时，有$\psi : GF \xrightarrow{\sim} \text{id}_{C_1}$并且存在$\theta : FG \xrightarrow{\sim} \text{id}_{C_2}$，即$\eta$和$\varepsilon$均为同构，这样可以引出范畴$C_1$和$C_2$的等价，由前文的讨论我们可以得知此时有$F$和$G$均为[全忠实，本质满函子](#范畴等价是全忠实，本质满函子).
...
若$\eta$是同构，则有$\eta_Y:Y \xrightarrow{\sim} GFY$使得$f \mapsto\eta_Y\circ f$给出双射$\text{Hom}(X,Y)\to \text{Hom}(X,GFY)$.由于$\varphi$是同构，于是有$f \mapsto \varphi^{-1}(\eta_Y\circ f) =$是双射，即$F$全忠实.
...
$$
是双射，由于$h_{C_1}$是全忠实函子，又有$\eta_Y f \in \text{Hom}(X,GFY)$于是得到$\eta$是同构.
