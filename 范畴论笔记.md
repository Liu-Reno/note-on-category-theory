...
对于$Y \in \text{Ob}(C_1)$，由于$GFY$可以看做一个集合，于是有$\eta_Y \in C^{\land}$.于是有
$$
\text{Hom}_{C^{\land}}(h_{C_1}(Y),\eta) &\to& \eta_Y\\
\left[\text{Hom}_{C_1}(\cdot,Y)\xrightarrow{\phi} \text{Hom}_{C_1}(\cdot,GF\cdot) \right]&\mapsto& \phi_Y(\text{id}_Y)
$$
为双射，于是若$\eta_Y: Y \to GFY$是同构，则有$f \mapsto \eta_Yf$给出$\text{Hom}(X,Y)\xrightarrow{\sim} \text{Hom}(X,GFY)$，既然$\varphi$是同构，则$f\mapsto Ff$是双射，即$F$为全忠实函子.
