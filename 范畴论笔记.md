...
对于范畴$\mathcal{C}$，取$X,Y,Z\in \text{Ob}(\mathcal{C})$，$f \in \text{Hom}_C(X,Y)$
...
* 考虑函子<img src="./image/7.png" alt="7" style="zoom: 50%;" />及态射$\theta : F_1 \to F_2$,$\psi : G_1 \to G_2$.今将定义横合成$\psi \circ \theta : G_1 \circ F_1 \to G_2\circ F_2$.首先注意到对所有的$X\in \text{Ob}(\mathcal{C}'')$，根据$\psi$的自然性，图表
...
4. 选定Grothendieck宇宙，证明其中全体有限全序集以及其间的保序映射构成一个范畴$\text{Ord}_f$.证明有限序数$\bold{0},\bold{1},\cdots$构成此范畴的骨架.
   ...
   接下来验证态射的合成，取$X,Y,Z\in \text{Ob(Ord}_f)$,令$f\in \text{Hom}(X,Y),g \in \text{Hom}(Y,Z)$有
   $$
...
2. 但是范畴的等价不止为本质满这一条性质，接下来对于态射集中对象之间的态射进行讨论：
   ...
   $\phi_{Y'}FGf'\phi^{-1}_{X'}$合成为恒等态射，于是有$FG \xrightarrow{\sim} \text{id}_{C_2}$，即$F$右可逆，$G$左可逆.(注记：$(\phi_Y)_*$以及$(\phi^{-1}_X)^*$为态射$\phi_Y$做推出与对态射$\phi^{-1}_X$做拉回，拉回和推出是我们对于两个对象间的态射集进行转化时的利器，在__函子范畴__中我们会对拉回与推出进行定义，见函子范畴的[Hom函子](#Hom函子).)
...
注意自同态$X \to X$不一定唯一，因$X$为对象可以看做某个集合.我们称$End(\text{id}_C)$为范畴$\mathcal{C}$中心$Z(\mathcal{C})$.
...
定义(__逗号范畴__) : 对于函子$\mathcal{A} \xrightarrow{S} \mathcal{\mathcal{C}} \xleftarrow{T} \mathcal{B}$，定义逗号范畴$(S/T)$(有些文献写为$(S\downarrow T)$)如下：
...
> 猜测：不难发现在逗号范畴中态射的合成借鉴了群的直积的构造，或者说逗号范畴实际上为一个积范畴$\mathcal{A} \times \mathcal{B}$的子范畴(大概率不是全子范畴)，限制方法为考虑一个中间范畴$\mathcal{C}$以及两个范畴到中间范畴$\mathcal{C}$的函子
...
把对于$\mathcal{C} \xrightarrow{U} \text{Set}$,$\mathcal{C}$中满足泛性质的对象称为自由对象(由于在可表函子中我需要用自由对象这一概念进行切入，故把自由对象放置在逗号范畴后，原位置应当为泛性质描绘后).

不难得到对于自由概念的描述:

由于一般用一个遗忘函子$U:\mathcal{C} \to \text{Set}$来构建泛态射$(X/U)$在集合范畴$\text{Set}$研究范畴$\mathcal{C}$内作为泛态射始对象的对象的性质，同理我们有时可以做出一个反向的函子$F:\text{Set} \to \mathcal{C}$（见[引例](#引例))进行反向地构造.这种函子我们称为自由(free)函子(自由群的构造就是一个典型的自由函子).

于是在接触自由函子之前，需要去对于遗忘函子进行定义， 

遗忘函子将范畴$\mathcal{C}$映射至范畴$\text{Set}$,显然有遗忘函子是一个忘却函子，回忆对于忘却函子的[定义](#忘却函子)可知遗忘函子是忠实的.

称能构造出遗忘函子的范畴$\mathcal{C}$为__具体(concrete)范畴__，并且$U:\mathcal{C} \to \text{Set}$为$\mathcal{C}$的一个遗忘函子

对于$\mathcal{C}$中的任意对象$X$,$UX$称为$X$的凭借(underlying)集合.

> 存在不具体的范畴：同伦范畴$\text{h Top}$，其中的对象是拓扑空间，态射为同伦类.
>
> 事实上此处的具体范畴意为可具体化的范畴，事实上具体范畴应当为一个范畴和一个遗忘函子的有序对$(\mathcal{C},U)$

既然有了遗忘函子，我们可以试着选定一个集合$X$，构建$X$到$U$的逗号范畴$(X/U)$.对于$(X/U)$中的始对象$(A,\phi:X \to UA)$,称$A$在集合$X$上自由.

多数情况下我们考虑的是$X \subset UA$并且$\phi = \iota$为包含态射的情况，这个时候称$A$为自由对象，在满足条件时可以反向地定义一个自由函子(见后文__伴随函子__中函子具有左伴随的条件).$F:\text{Set} \to V$，自由函子将集合$X$映射为$X$所生成的自由对象$FX$，而将映射$f:X \to Y$自然扩张为$Ff :FX \to FY$.



### 总结

在构造解决问题所需要的数学结构时不妨考虑泛性质，构造一个函子将两个范畴联系起来(一般是一个范畴$\mathcal{C}$到集合范畴$\text{Set}$)，针对这个函子与目标范畴的某个对象$X$做逗号范畴$(X/U)$，通过研究逗号范畴的始对象来研究数学结构.
...
3. 试构造逗号范畴$(\text{id}_C/\text{id}_C)$.
   ...
   其对象即为$\mathcal{C}$中的所有态射$f : X \to Y$
...
5. 设$\mathcal{\mathcal{C}}$为范畴，并对每个$X,Y\in \text{Ob}(\mathcal{\mathcal{C}})$在$\text{Hom}_{\mathcal{C}}(X,Y)$上给定二元关系$\mathcal{R}$.构造相应的__商范畴__ $\mathcal{C/R}$连同函子$Q: \mathcal{C}\to \mathcal{C/R}$使得

   * 对任意$\mathcal{C}$中的态射$f,g$都有$f\mathcal{R}g \Rightarrow Q(f) = Q(g)$,
   * 函子$Q$在对象集上是双射,
   * 对任何函子$S : \mathcal{C}\to \mathcal{C}'$满足$f \mathcal{R}g\Rightarrow S(f)=S(g)$者，存在唯一的函子$\overline{S} : \mathcal{C/R} \to \mathcal{C}'$使得$S = \overline{S}Q$.

   说明$Q:\mathcal{C}\to \mathcal{C/R}$的唯一性.

   

   提示: 

   假设存在一个$P:\mathcal{C} \to \mathcal{C/R}$也满足以上性质，于是$P$在对象集上也是双射，且对有$S: \mathcal{C}\to \mathcal{C}'$满足$f\mathcal{R}g \Rightarrow S(f)=S(g)$存在唯一的$\overline{S}': \mathcal{C/R}\to \mathcal{C}'$使得$S = \overline{S}'P$

   由$S = \overline{S}Q$有$\overline{S}Q(f) = S(f) = \overline{S}'P(f)$.

   接下来先对于商范畴进行分析.

   由于有$f\mathcal{R}g \Rightarrow Q(f) = Q(g),P(f) = P(g)$于是可以得到态射$f$的等价类$[f]$.有对于任意的$g,f \in [f],P(f)=P(g),Q(f) = Q(g)$.

   根据$S$所满足的条件可知，其将$[f]$映射至一个态射$f' \in \mathcal{C}'$上.

   于是对于$X,Y \in \text{Ob}(\mathcal{C})$,令$f\in \text{Hom}(X,Y)$有$f' \in \text{Hom}(SX,SY)$.不难发现$[f]$是$\text{Hom}(X,Y)$的一个子集.

   由于$f\mathcal{R}g$可以推出$Q(f) = Q(g)$不难得出$g\mathcal{R}f$也可以推出$Q(f) = Q(g)$更近一步可以验证其为一个等价关系，于是它可以将$\text{Hom}(X,Y)$划分为若干个等价类.

   由于$Q$在对象集上为双射，于是其实际上是将态射集$\text{Mor}(\mathcal{C})$映射为关于$\mathcal{R}$的全部等价类的集合$\text{Mor}(\mathcal{C/R})$.

   于是不难构建一个样例的$Q$，即
   $$
   Q: \mathcal{C} &\to& \mathcal{C}':=\mathcal{C/R}\\
   X &\mapsto& X\\
   f &\mapsto& [f]
   $$
   不难验证若有其他的$P$满足以上的条件，考虑(包含所有范畴的)范畴$\text{Cat}$与范畴$\text{Cat}$之间的函子$U$，考虑$\mathcal{C},\mathcal{C/R}\in \text{Ob(Cat)}$，有$(Q,S: C \to C')$为其始对象，根据泛性质得到$Q$(在至多相差一个同构的情况下)唯一.

   

6. 试证明：若集合$X_1$和$X_2$同构，则$\mathcal{C}$中的对象$A_i$在$X_i$上自由$(i \in \{1,2\})$，则$A_1$和$A_2$同构.
...
想要研究结构的结构就需要将结构内部的对象也进行研究，那如何去研究范畴$\mathcal{C}$内部的对象$X$呢，不难想到我们在上一章泛性质中所提到的[自由对象](#自由对象)以及[$\text{Hom}$函子](#\text{Hom}函子)，发现$\text{Hom}$函子也可以视为一个范畴$\mathcal{C}^{op}\times \mathcal{C}$到集合范畴$\text{Set}$的函子，可以对其稍作变换变为范畴$\mathcal{C}$到集合范畴$\text{Set}$的函子.由于$\text{Hom}$函子将$(X,Y)$映射为$\text{Hom}(X,Y)$于是如果打算用其来表示$X$，就需要对其进行一些细微的变动.

### 预层范畴

通过泛性质对于[自由对象](#自由对象)的描述，不难发现自由对象是建立在范畴$\mathcal{C}$与集合范畴$\text{Set}$之间的，忘却函子是一个由$\mathcal{C}$到$\text{Set}$的忠实函子，于是考虑反变函子，显然有所有的反变函子$F$都有$F\in \text{Fct}(\mathcal{C}^{op},\text{Set})$.于是研究泛性质所依靠的函子本质上就是在研究$\text{Fct}(\mathcal{C}^{op},\text{Set})$的性质.

> 为何预层为$\text{Fct}(\mathcal{C}^{op},\text{Set})$？首先，$\mathcal{C}^{op}$与$\mathcal{C}$的结构互为对偶，研究$\text{Fct}(\mathcal{C}^{op},\text{Set})$即研究$\text{Fct}(\mathcal{C},\text{Set})$其次对于$F: \mathcal{C}^{op} \to \text{Set}$,$f : X\to Y$，有$F f : FY \to FX$进行了一个倒转，于是对于$\text{Hom}$集合利用$f$进行推出或者拉回时，经过自然变换变为$F$仍然可以使用$Ff$对于$FX$和$FY$进行研究，主要是便于证明米田引理，这方便我们进行后续研究.

这个函子范畴相当于连接范畴$\mathcal{C}$这一层面和最基本的层面(集合范畴)$\text{Set}$，于是这个函子可以称为$\mathcal{C}$的预层(presheaf)范畴(来源于几何学的层论)

我们将预层范畴$\text{Fct}(\mathcal{C}^{op},\text{Set})$记为$\mathcal{C}^{\land}$.同理，可以定义$\mathcal{C}^{lor} :=\text{Fct}(\mathcal{C}^{op},\text{Set}^{op}) = \text{Fct}(\mathcal{C},\text{Set})^{op}$.不难发现$(\mathcal{C}^{\lor})^{op} = (\mathcal{C}^{op})^{\land}$.

### $h_C$函子与米田(Yoneda)引理

正如本章前言所述，需要利用$\text{Hom}$函子来表示$X$，由于$\text{Hom}:\mathcal{C}^{op} \times \mathcal{C} \to \text{Set}$，对于$(X,Y) \to \text{Hom}(X,Y)$固定$Y$的情况下有对于任意的$X \in \text{Ob}(\mathcal{C})$，考虑其全体就可以得到所有映射向$Y$的态射$t^{-1}(Y) = \text{Hom}(\cdot,Y)$，如果我们把任意一个$X \in \text{Ob}(\mathcal{C})$放入$\cdot$中，自然得到$\text{Hom}(X,Y) \in \text{Set}$，于是就可以得出一个$\mathcal{C}$向$\mathcal{C}^{\land}$的函子$h_C$.
$$
h :& C \to& C^{\land}\\
...
\\&S\times A \to A(S)
$$
 由于对于$h_T$有$(S,h_C(T)) \to \text{Hom}(S,T)$，于是由于$\text{Hom}$函子的[注记](#\text{Hom}函子注记)知$\text{ev}^{\land}:\mathcal{C}^{op} \times \mathcal{C}^{\land}\to \text{Set}$.
...
接下来对于$h_C$的性质进行探索，由于$\mathcal{C}^{\land}$中可能存在函子$F$使得对于任意的$S \in \mathcal{C}$,没有$F \simeq h_C(S)$，于是$h_C$不为满函子，接下来我们对其全忠实性进行考虑.
...
引理(__米田信夫(Yoneda)__) :对于$S \in \text{Ob}(\mathcal{C})$和$A \in \text{Ob}(\mathcal{C}^{\land})$ ，有映射
$$
...
u \mapsto \Phi(u)
$$
对于所有的$T \in \text{Ob}(\mathcal{C})$,$f \in \text{Hom}(T,S)$有$\Phi(u)_T(f) = A(f)(u)$.

证明 : 

1. 证明正向可定义，对于任意的$\phi \in \text{Hom}_{\mathcal{C}^{\land}}(h_C(S),A)$，有$\phi_S(\text{id}_S)\in A(S)$，并且$\phi_S(\text{id}_S)$唯一，于是正向映射可定义.
...
既有$h_C$为全忠实函子，于是对$\mathcal{C}^{\land}$做出以下限制$h_C(\text{Ob}(\mathcal{C})) = \text{Ob}(\mathcal{C}'^{\land})$,取全子范畴即可得知$\mathcal{C}'^{\land}\xrightarrow{\sim} \mathcal{C}$，即$h_C$为该全子范畴到$\mathcal{C}$的范畴等价，由$h_C$所确定的全子范畴可以将$\mathcal{C}$中的对象$S$转化到$\mathcal{C}^{\land}$中，并且$S$可以由$h_C(S)$唯一确定，即$h_C(S)$可以表出范畴$\mathcal{C}$中的对象$S$.<a id="对象表出"></a>

对于$h_C$这一类函子以及与其同构的函子我们称为可表函子.以下是可表函子的严格定义

定义(__可表函子__) : 称$A: \mathcal{C}^{op}\to \text{Set}$是可表函子，如果存在$X \in \text{Ob}(\mathcal{C})$及同构$\phi : h_C(X) \xrightarrow{\sim} A$，并称$(X,\phi)$是代表元.类似地，可以用$k_C$定义函子$B : \mathcal{C} \to \text{Set}$的可表性和代表元.
...
定义逗号范畴$(h_C/A) : \mathcal{C} \xrightarrow{h_C} \mathcal{C}^{\land} \xleftarrow{j_A} 1$.$(S,\psi : h_C(S) \to A)$(即对偶的泛态射).

对于任意的$S \in \text{Ob}(\mathcal{C})$与$\varphi : h(S) \to A$，由于$h_C$为全忠实函子，于是存在唯一的$f$使下图交换
...
可表函子是一种将范畴$\mathcal{C}$映射到集合范畴$\text{Set}$的函子并且其与某个$S \in \text{Ob}(\mathcal{C})$经过$h$函子变换后$h(S)$同构.

泛态射与伴随函子也可以用可表函子来表示.

令$G : \mathcal{C}' \to \mathcal{C}$是一个函子并且$X$是$\mathcal{C}$中的一个对象，于是$(A, \varphi)$是一个$X$到$G$的泛态射当且仅当$A$是$\text{Hom}_C(X,G(\cdot)) : \mathcal{C}' \to \text{Set}$.若$\text{Hom}_C(X,G(\cdot))$对于$\mathcal{C}$中所有的$X$可表(即对于所有的$X$都有$\text{Hom}_C(X,G(\cdot)$为可表函子，在伴随函子中我们会提到)，则$G$具有左伴随.

自然同构$\Phi_X:\text{Hom}_{\mathcal{C}'}(FX,\cdot) \to \text{Hom}_{\mathcal{C}}(X,G(\cdot))$产生了伴随性，即
$$
\Phi_{X,Y}: Hom_{C'}(FX,Y) \to Hom_C(X,GY)
$$
对于所有的$X$和$Y$都是双射.

这个命题的对偶命题仍为真.设$F : \mathcal{C} \to \mathcal{C}'$是函子，$Y$是$\mathcal{C}'$的对象，则$(a,\varphi)$是$F$到$Y$的泛态射当且仅当$(a,\varphi)$是函子$\text{Hom}_{\mathcal{C}'}(F(\cdot),Y) :\mathcal{C} \to \text{Set}$的代表元.由此得出$F$有右伴随当且仅当对与所有的$Y \in \text{Ob}(\mathcal{C}')$都有$\text{Hom}_{\mathcal{C}'}(F(\cdot),Y)$可表.
...
3. 使用Yoneda引理证明Cayley定理.
   ...
   考虑只有一个对象$*$的广群$\mathcal{C}$,于是有$End(*,*)$是群$G$，对于函子$F : \mathcal{C} \to \text{Set}$，由一个集合$X$和一个群同态$G \to Sym(X)$,$Sym(X)$是$X$的一个对称群，换句话说，$X$是一个$G$集，对于$G$集之间的保构态射$\alpha :X \to Y$有性质$\alpha (g \cdot x) = g \alpha(x)$对于所有的$g \in G$和$x \in X$.

   考虑$h_C(*)$和群$G$到自身($Sym(G)$)的作用,由米田引理有$\text{Hom}_{\mathcal{C}^{\land}}(h_C(*),h_C(*)) \simeq End(*)$.
...
在[范畴之间的等价与同构](#范畴之间的等价与同构)(以及其前一节)一节中我们阐述了函子同构以及范畴等价的定义
...
不难发现在范畴等价上其构建了一个自然变换$\psi:GF \xrightarrow{\sim}\text{id}_{C_1}$.接下来对其进行拓展.

对于范畴$C_1，C_2$构建一对函子<img src="./image/33.png" alt="image-20230621181048302" style="zoom:33%;" />(要求$F$与$G$同构)并且延续先前对于范畴等价的定义可以进行如下拓展.
...
接下来我们研究函子$F$与$G$之间的关系(我们先前已然假设$F$与$G$是同构的).
...
[例] 选定域$\mathbb{k}$.我们已经定义了[对偶函子](#对偶函子)$D: \text{Vect}(\mathbb{k})^{op} \to \text{Vect}(\mathbb{k})$，$DV := V^{\lor}$.存在自然的同构
$$
\varphi_{V,W}:\text{Hom}_{\mathbb{k}}(V,W^{\lor}) &\to& \text{Hom}_{\mathbb{k}}(W,V^{\lor})\\
 f &\mapsto& [w \mapsto [v \mapsto f(v)(w)]]
$$
其中$V,W$是$\mathbb{k}$-向量空间.实际上，两边都自然地同构于双线性型$B : V \times W \to \mathbb{k}$构成的空间:仅需将$f\in \text{Hom}_{\mathbb{k}}(V,W^{\lor})$映射到$B : (v,w)\mapsto f(v))(w)$.交换$V,W$的角色就可以得到$\text{Hom}_{\mathbb{k}}(W,V^\lor)$的情形.这些同构可以重写为
$$
\varphi_{V,W}:\text{Hom}_{\mathbb{k}}(V,DW) \xrightarrow{\sim} \text{Hom}_{\text{Vect}(\mathbb{k})^{op}}(D^{op}V,W)
$$
且对于变元$V,W$满足函子性，因此可以得到伴随对$(D^{op},D,\varphi^{-1})$.
...
$$
至于上述关系，我们也可以使用$2$-胞腔的写法概括.将横合成$\eta G = \eta \circ \text{id}_G: G \to GFG$表作

![image-20230624130534490](./image/40.png)

类似地描绘$G\varepsilon$,$F\eta$,$\varepsilon F$等图表，于是我们可以将上述关系写为

![image-20230624130646105](./image/41.png)

![image-20230624130710853](./image/42.png)

也可以写为如下的便于记忆的形式

![image-20230624130757335](./image/43.png)

![image-20230624130820589](./image/44.png)
...
因此伴随对$(F,G,\varphi)$亦可以用$(F,G,\eta,\varepsilon)$来描述，这样就可以不牵扯到$\text{Hom}$集.



由于$C_1$和$C_2$等价时，有$\psi : GF \xrightarrow{\sim} \text{id}_{C_1}$并且存在$\theta : FG \xrightarrow{\sim} \text{id}_{C_2}$，即$\eta$和$\varepsilon$均为同构，这样可以引出范畴$C_1$和$C_2$的等价，由前文的讨论我们可以得知此时有$F$和$G$均为[全忠实，本质满函子](#范畴等价是全忠实，本质满函子).

那么我们不禁思考，当$\eta$和$\varepsilon$只有一个为同构时，会发生什么呢？

命题: 设$(F,G,\varphi)$是伴随对，则$\eta$(或$\varepsilon$)是同构当且仅当$F$($G$)是全忠实函子.

证明: 基于对偶性(使用$C_i^{op}$代替$C_i$)可以发现我们只需要证明$\eta$的情况，$\varepsilon$的就是自明的.

考虑交换图表

![image-20230624010339713](./image/39.png)

即可得到$\varphi(Ff) = \eta_Y f$.

若$\eta$是同构，则有$\eta_Y:Y \xrightarrow{\sim} GFY$可以得到$f \mapsto\eta_Y\circ f$给出双射$\text{Hom}(X,Y)\to \text{Hom}(X,GFY)$.由于$\varphi$是同构，于是有$f \mapsto \varphi^{-1}(\eta_Y\circ f) = Ff$是双射，即$F$全忠实.



若$F$是全忠实函子，于是对于任意的$X,Y \in \text{Ob}(C_1)$有
$$
\text{Hom}(X,Y) \to \text{Hom}(FX,FY)
$$
是双射，由于$\varphi$是同构，于是根据上图可以将其转化为
$$
\text{Hom}(X,Y) \to \text{Hom}(X,GFY)
$$
是双射.

由前文的讨论得知又有$\eta_Y f \in \text{Hom}(X,GFY)$,即上述映射正是$\eta_Y$在$\text{Hom}_C(X,\cdot)$下的像，但$X$任意选取.

即
$$
h_{C_1}(Y)\to h_{C_1}(GFY)
$$
由于$h_{C_1}$是全忠实函子(米田引理)，于是有$h_{C_1}$在态射集上为双射，即$h_{C_1}(GFY) \to h_{C_1}(Y)$,于是得到$\eta$是同构.$\Box$

不难发现若$F$和$G$均为全忠实函子时有$\eta$和$\varepsilon$均为同构，亦即$F$与$G$均为全忠实本质满函子.



数学中有许多构造是忘却函子或者包含函子的左伴随.以下举少量例子.

1. 忘却函子$\text{Grp} \to \text{Set}$的左伴随是自由群函子$F: X\mapsto \bold{F}(X)$,其单位是集合$X$到其自由群的嵌入$X \hookrightarrow \bold{F}(X)$.以后我们还会考察自由模和自由环等类似的自由构造，无论对于哪一种结构，准则不外乎是__"自由是遗忘的左伴随"__.
2. 包含函子$\text{Ab} \to \text{Grp}$的左伴随是$\text{Abel}$化函子$G \mapsto G/G_{der}$，其单位是商同态$G \to G/G_{der}$.
3. 包含函子$\text{ComMetr}\to \text{Metr}$的左伴随是完备化$(X,d)\mapsto (\hat{X},\hat{d})$，其单位是标准(对角)的等距嵌入$X \hookrightarrow \hat{X}$.
...
在先前的讨论中，我们先定义了一个相互同构的函子$F$和$G$，而不是给出任意一个函子$F:C_1\to C_2$判断其是否具有右伴随，接下来我们试着研究$F$具有右伴随的条件.

 考虑$\text{Hom}$函子$\text{Hom}_{\text{Cat}}(\cdot,\text{Set})$作用于$F$得到预层范畴上的函子
$$
A: C_2^{\land} \to C_1^\land
$$
具体言之，对于函子$B : C_2^{op}\to \text{Set}$和$C$中的对象$X$有
$$
(AB)(X) =B(FX)
$$
使用Yoneda引理可以[将$C_2$视为$C_2^\land$的全子范畴](#对象表出)，于是可以将$F_*$限制在$C_2$上(即考虑复合$A:=F_*\circ h_{C_2}$)得到
$$
A : C_2 \to C_1^{\land}
$$
其将$Y\in C_2$映射至$\text{Hom}_{C_2}(F(\cdot),Y)\in C_1^{\land}$.即有
$$
A_Y :=\text{Hom}_{C_2}(F(\cdot),Y)\in C_1^{\land}
$$


我们做出如下断言:

断言 : 函子$F:C_1 \to C_2$有右伴随的充分必要条件是$A_Y$对于每一个$Y$皆可表；类似地$G:C_2\to C_1$有左伴随的充分必要条件是$\text{Hom}_{C_1}(X,G(\cdot))$对于，每个$X$皆可表.

证明: 

由对称性可以得知只需要证明$F$的情况,

($\Rightarrow$)若$F$具有右伴随，则存在$G:C_2 \to C_1$使得$\text{Hom}_{C_2}(F(\cdot),Y)\xrightarrow{\sim}\text{Hom}_{C_1}(\cdot,GY)=h_{C_1}(GY)$.即$AY$对于每个$Y$皆可表.

($\Leftarrow$)现假设对每个$Y$都存在对象$GY$和同构$\psi_Y :h_{C_1}(GY)\xrightarrow{\sim}A_Y$.留意到$h_C(GY) = \text{Hom}_{C_2}(\cdot,GY)$.由于$(GY,\psi_Y)$是逗号范畴$(h_{C_1}/A_Y)$的终对象.于是有$(GY,\psi_Y)$在至多相差一个唯一的同构的条件下是唯一的.

接下来对于每个$Y$选定$(GY,\psi_Y)$，任意态射$h : Y \to Y'$诱导$h_*:A_Y \to A_{Y'}$因而存在唯一的$Gh : GY \to GY'$使得下图交换

<img src="./image/45.png" alt="image-20230624163920717" style="zoom:50%;" />

由此易知$(GY,\psi_Y)_Y$给出$G:C_2 \to C_1$和$\varphi = \psi^{-1}: \text{Hom}(F(\cdot),\cdot) \xrightarrow{\sim}\text{Hom}(\cdot,G(\cdot))$.$\Box$



命题 : 函子$F:C_1\to C_2$若有右伴随，则在下述意义下唯一: 若$(F,G,\varphi)$和$(F',G',\varphi')$是伴随对，则存在唯一的同构$\psi : G\xrightarrow{\sim} G'$使得对每个$Y \in \text{Ob}(C_2)$满足下图交换

<img src="./image/46.png" alt="image-20230624164629041" style="zoom:67%;" />

左伴随也满足类似的唯一性.

证明: 我们在上一个命题中的证明已然说明了$(GY,\psi_Y)$和$(G'Y,\psi_Y')$在$(h_{C_1},A_Y)$中的终对象，于是由于[终对象之间必然同构](#终对象之间同构)可以对于每个$Y$制造同构$\psi_Y : GY \xrightarrow{\sim}G'Y$，并且使用终对象的泛性质得到$(\psi_Y)_Y$为一个自然变换.$\Box$
...
|          $j_X$          | 从$1$范畴到$\mathcal{C},X \in \text{Ob}(\mathcal{C})$的函子有$ 0 \mapsto X$ | j_X                 |
